...
class V1EnterpriseAllocationGetResponse(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of consumption_summary
        if self.consumption_summary:
            _dict['consumptionSummary'] = self.consumption_summary.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each value in conversion_holders (dict)
        _field_dict = {}
        if self.conversion_holders:
            for _key_conversion_holders in self.conversion_holders:
                if self.conversion_holders[_key_conversion_holders]:
                    _field_dict[_key_conversion_holders] = self.conversion_holders[_key_conversion_holders].to_dict()
            _dict['conversionHolders'] = _field_dict
        # override the default output from pydantic by calling `to_dict()` of each item in regional_allocations (list)
        _items = []
        if self.regional_allocations:
            for _item_regional_allocations in self.regional_allocations:
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "consumptionSummary": ManaV2BandwidthConsumptionSummary.from_dict(obj["consumptionSummary"]) if obj.get("consumptionSummary") is not None else None,
            "conversionHolders": dict(
                (_k, ManaV2AllocationConversionHolder.from_dict(_v))
                for _k, _v in obj["conversionHolders"].items()
            )
